...
internal class EditorGizmo(
    ...
    fun render() {
        ...
        if (collision != null) {
            position = transform.position.add(rotateEuler(collision.position.multiply(parentScale), transform.rotation))
            rotation = transform.rotation.add(collision.rotation)
    ...
    private fun applyCollisionDelta(
        ...
        when (context.gizmoOperation) {
            Operation.TRANSLATE -> {
                val localDelta = rotateEuler(
                    Vector3f(deltaPosition[0], deltaPosition[1], deltaPosition[2]),
                    parentRotation.scale(-1f),

...
object GltfLevelLoader {
    private const val JSON_CHUNK_TYPE = 0x4E4F534A

    fun loadIfPresent(resourcePath: String): GltfLevel? {
        return if (ClassLoader.getSystemResource(resourcePath) == null) {
            Logger.info("glTF level not found, skipped: {}", resourcePath)
            null
        } else {
            load(resourcePath)
        }
    ...
    fun load(resourcePath: String): GltfLevel {
        ...
        var terrain: GltfTerrain? = null
        val staticMeshes = mutableListOf<ObjectEntity>()
        val entitySpawns = mutableListOf<GltfEntity>()
        val areaTriggers = mutableListOf<GltfTrigger>()

        gltfModel.nodeModels.forEachIndexed { index, node ->
            val meta = metadata.getOrNull(index) ?: GltfNodeMetadata(node.name ?: "node_$index")
        ...
        Logger.info(
            "glTF level loaded: {} (terrain={}, staticMeshes={}, entities={}, areaTriggers={})",
            resourcePath,
            terrain != null,
            staticMeshes.size,
            entitySpawns.size,
    ...
    private fun buildTerrain(
        ...
        Logger.info("glTF terrain loaded from mesh node {}", name)

        return GltfTerrain(
            entity = entity,
            triangles = geometry.triangles.map { triangle ->
                GltfTerrainTriangle(triangle.a, triangle.b, triangle.c)
            },
        )
...
//        val minHeight = extras.float("minHeight") ?: 1f
    ...
    private fun buildCollider(
        type: GltfColliderType,
        vertices: List<Vector3f>,
        triangles: List<Triangle>,
    ): Collider? {
        if (type == GltfColliderType.NONE || vertices.isEmpty()) return null

        return when (type) {
            GltfColliderType.AABB -> buildBoxCollider(vertices)
            GltfColliderType.CONVEX_HULL -> buildConvexHullCollider(vertices, triangles)
            GltfColliderType.NONE -> null
        }
    }
    ...
    private fun transformFromMatrix(matrix: FloatArray): Transform {
        ...
        return Transform(
            position = Vector3f(matrix[12], matrix[13], matrix[14]),
            rotation = rotation,
    ...
    private fun colliderType(extras: Map<String, Any?>): GltfColliderType {
        return when ((extras.string("colliderType") ?: extras.string("collider") ?: "none").normalized()) {
            "aabb", "box", "boxcollider" -> GltfColliderType.AABB
            "convexhull", "convexhullcollider", "convex" -> GltfColliderType.CONVEX_HULL
            else -> GltfColliderType.NONE
        }
    }
    ...
    private fun readNodeMetadata(resourcePath: String): List<GltfNodeMetadata> {
    ...
    private fun logDebugJsonSummary(resourcePath: String) {
        ...
        Logger.debug("glTF JSON summary: {}", resourcePath)
    ...
    private fun readJsonRoot(resourcePath: String): Map<*, *>? {
        ...
        return GltfJson.parse(jsonText) as? Map<*, *>
    }
    ...
    private data class GltfNodeMetadata(
        val name: String,
